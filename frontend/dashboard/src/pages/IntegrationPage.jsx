...
function WebhookSettingsPanel({ integration, onUpdated, disabled = false }) {
  ...
  async function copyBearerToken() {
    ...
    try {
      ...
      setError('Could not copy the webhook token. Select the token and copy it manually.');
    }
  }
    ...
    <div className="space-y-4">
      <div>
        ...
        <FieldHint>
          ...
          {' '}Save the token generated by Priority below. The service verifies every request against that saved value.
        </FieldHint>
      </div>
      <div className="flex items-center gap-2 text-xs text-slate-500">
        <span>Priority webhook token</span>
        {settings?.secretTokenReference ? <Badge value="active">saved</Badge> : <Badge value="inactive">not set</Badge>}
      </div>
      {!disabled && (
        <div>
          <label className="block text-xs font-medium text-slate-600">Saved Priority token</label>
          <div className="mt-1 flex gap-2">
            <code className="block min-w-0 flex-1 overflow-x-auto rounded-md border border-slate-200 bg-white px-3 py-2 text-sm">
              {loadingToken ? 'Loading token...' : webhookToken || 'No token saved yet'}
            </code>
            <button type="button" onClick={copyBearerToken} disabled={!webhookToken} className="rounded-md border border-slate-300 px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 disabled:opacity-50">
              {copiedToken ? 'Copied' : 'Copy'}
            </button>
          </div>
          <FieldHint>Priority sends this as the Priority-BPM-Token header. For manual testing, you can also use Authorization Bearer Token.</FieldHint>
        </div>
      )}
      ...
      <FieldHint>Saving a token replaces the value that incoming Priority webhooks must match.</FieldHint>
