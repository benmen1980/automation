...
function WebhookSettingsPanel({ integration, onUpdated, disabled = false }) {
  ...
  async function loadWebhookToken() {
    if (disabled || integration.type !== 'webhook') return;
    setLoadingToken(true);
    try {
      const result = await api.integrations.webhookToken(integration.id);
  ...
  async function rotateToken(e) {
    ...
    try {
      ...
      setSavedMessage('Priority BPM token saved to the database. Incoming webhooks must send this exact value in the Priority-BPM-Token header.');
  ...
  async function copyPriorityHeader() {
    setError('');
    if (!token) return;
    try {
      await navigator.clipboard.writeText(`Priority-BPM-Token: ${token}`);
      setCopiedHeader(true);
    } catch {
      setError('Could not copy the header. Select the token and copy it manually.');
    }
  }
    ...
    <div className="space-y-4">
      <div>
        ...
        <FieldHint>
          ...
          {' '}Priority must send the token below in the Priority-BPM-Token header.
        </FieldHint>
      </div>
      <form onSubmit={rotateToken} className="space-y-2">
        <div className="flex items-center gap-2 text-xs text-slate-500">
          <span>Priority BPM token</span>
          {settings?.secretTokenReference || token ? <Badge value="active">saved</Badge> : <Badge value="inactive">not set</Badge>}
        </div>
        <div className="flex gap-2">
          <input
            type="text"
            value={loadingToken ? 'Loading token...' : token}
            onChange={(e) => setToken(e.target.value)}
            placeholder={disabled ? 'Token changes disabled for viewer role' : 'Paste token generated by Priority'}
            disabled={disabled || loadingToken}
            className="min-w-0 flex-1 rounded-md border border-slate-300 px-3 py-2 font-mono text-sm"
          />
          <button type="button" onClick={copyPriorityHeader} disabled={!token || loadingToken} className="rounded-md border border-slate-300 px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 disabled:opacity-50">
            {copiedHeader ? 'Copied' : 'Copy header'}
          </button>
          <button type="submit" disabled={saving || !token || disabled || loadingToken} className="rounded-md bg-[#306cb4] px-3 py-2 text-sm font-semibold text-white hover:bg-[#028baa] disabled:opacity-50">
            {saving ? 'Saving...' : 'Save'}
          </button>
        </div>
      </form>
      <FieldHint>
        This is one visible integration parameter. Rejected token attempts are logged under Logs as warnings, not Recent executions, because no execution is created until the token matches.
      </FieldHint>
      <div className="rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-xs leading-5 text-slate-600">
        ...
        <p className="mt-1">Header: <code>Priority-BPM-Token: {token || '<saved Priority token>'}</code></p>
        <p>Body must include <code>{'{"CPROF":{"CPROFNUM":"PQ26000001","CDES":"Customer name"}}'}</code>. An empty body will validate the token first, then fail with a clear payload error.</p>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(360px,0.65fr)]">
        <div className="space-y-6">
          ...
          <Card title="Credentials" description="Only fields declared by this integration are shown. Visible parameters stay visible after saving; logs still redact sensitive-looking values.">
            {!canManage && <ReadOnlyNotice />}
        ...
        <aside className="space-y-6">
          {connectorOptions.length > 0 && (
            <Card title="Credential test" description="Choose which external account to verify. The test uses the saved database values and shows a clear provider response.">
              ...
              {activeConnector && (
                <FieldHint>
                  Testing {connectorLabel(definition, activeConnector)} uses the saved values for this integration and keeps tokens out of logs.
                </FieldHint>
              )}
