...
function normalizeRecipientPhone(phone) {
  ...
  if (!raw) throw new Error('Priority webhook payload is missing DOCUMENTS_D.YARD_PHONENUM.');
  ...
  if (!/^\+[0-9]{8,15}$/.test(normalized)) {
    throw new Error('DOCUMENTS_D.YARD_PHONENUM must be a valid Israeli or E.164 phone number.');
  }
  return normalized;
}
...
function getDocumentFields(payload) {
  const order = payload?.DOCUMENTS_D;
  if (!order || typeof order !== 'object' || Array.isArray(order)) {
    throw new Error('Priority webhook payload must include an DOCUMENTS_D object.');
  }
  const hasPrimaryRecipient = Boolean(String(order.YARD_CUSTDES || '').trim() && String(order.YARD_PHONENUM || '').trim());
  const hasFaxRecipient = Boolean(String(order.YARD_NAME || '').trim() && String(order.YARD_FAX || '').trim());
  const fax = hasFaxRecipient && order.YARD_FAX && typeof order.YARD_FAX === 'object'
    ? requiredText(order.YARD_FAX, 'DOCUMENTS_D.YARD_FAX')
    : String(order.YARD_FAX || '').trim();
  return {
    orderName: requiredText(order.ORDNAME, 'DOCUMENTS_D.ORDNAME'),
    date: rawDateText(order.CURDATE),
    customerDescription: hasPrimaryRecipient ? requiredText(order.YARD_CUSTDES, 'DOCUMENTS_D.YARD_CUSTDES') : '',
    recipientPhone: hasPrimaryRecipient ? normalizeRecipientPhone(order.YARD_PHONENUM) : '',
    faxCustomerDescription: hasFaxRecipient ? requiredText(order.YARD_NAME, 'DOCUMENTS_D.YARD_NAME') : '',
    faxRecipientPhone: hasFaxRecipient ? normalizeRecipientPhone(fax) : '',
  };
}

function mapDocument(payload, credentials = {}, overrides = {}) {
  const { orderName, date, customerDescription, recipientPhone } = getDocumentFields(payload);
