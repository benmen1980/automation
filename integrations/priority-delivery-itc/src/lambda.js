...
function lifecycleDetails(job) {
  return {
    integrationName: job.integrationName || 'priority-delivery-itc',
...
async function processJob(job, recordId, runtime = {}) {
  ...
  const logger = createLogger({ service: 'priority-delivery-itc', jobId });
  ...
  logger.info('Priority delivery to ITC execution started.', {
    ...details,
    status: 'running',
    startedAt,
  ...
  try {
    ...
    if (priorFinalization?.state === 'SUCCESS') {
      ...
      logger.info('Priority delivery to ITC execution finalized without resending.', {
        ...details,
        status: 'success',
        finishedAt: priorFinalization.finishedAt,
        idempotentSkip: true,
    ...
    if (priorFinalization?.state === 'FAILED') {
      ...
      logger.error('Priority delivery to ITC terminal failure finalized without resending.', {
        ...details,
        status: 'failed',
        finishedAt: priorFinalization.finishedAt,
        finalizationOnly: true,
    ...
    if (claim?.alreadyCompleted) {
      logger.info('Priority delivery to ITC execution skipped as already completed.', {
        ...details,
        status: 'success',
        finishedAt: new Date().toISOString(),
        idempotentSkip: true,
    ...
    logger.info('Priority delivery to ITC execution finished successfully.', {
      ...details,
      status: 'success',
      finishedAt,
      ...(result.counts || {}),
    ...
    if (retryableFailure && attemptNumber < maxReceiveCount) {
      ...
      logger.error('Priority delivery to ITC execution will retry before any accepted delivery.', {
        ...details,
        ...normalizeError(error),
        providerError: error.providerError,
        status: 'retrying',
    ...
    logger.error('Priority delivery to ITC execution failed.', {
      ...details,
      ...normalizeError(error),
      providerError: error.providerError,
      status: 'failed',
